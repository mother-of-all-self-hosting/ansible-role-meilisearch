...
#
# Tags look like `v<Meilisearch version>-<release>`, which is what this
# repository has always published (`v1.48.3-0`, `v1.48.3-1`, `v1.50.0-0`):
#
# - if defaults/main.yml points at a Meilisearch version that has never been
#   released, the release counter restarts at 0 (`v1.53.1-0`)
# - otherwise the counter is incremented (`v1.53.1-1`), but only if something
...
# Anchored on `meilisearch_version:` so that neither the
# `meilisearch_container_image_tag:` line that refers to it nor a commented-out
# line can be mistaken for it.
version="$(sed -nE 's|^meilisearch_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Meilisearch version from $defaults_path"
	exit 1
fi

# The version values already carry a leading `v` (e.g. `v1.53.1`),
